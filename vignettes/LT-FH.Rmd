...
```{r}
LTFH_summary = GWAS(G=G, y=LTFH_est$l_g_0, p=5e-7, logreg = FALSE, ncores=3)
head(LTFH_summary)
```

In the above table we se the result of the association analysis, where we can se which SNP's is estimated as casual. We are here using bonferronni correction to lower the amont of false positive causal SNP. And since we have $100,000$ indiviuals we will use $p=\frac{0.05}{100000}=5e-7$.
...
![Scatterplot plot of LTFH used on 100,000 times 100,000 data][LTFH_scatter.jpg)
![Powerplot of LTFH used on 100,000 times 100,000 data][LTFH_power.jpg)
