...
Here the effect of a each causal snip is of the distribution $\mathcal{N}\left( 0, \frac{h^2}{c}\right)$ and each minor allele is sampled from the distribution $unif(0.01, 0.49)$. Using the function dist_check() we can see whether the distribution of the full liabilities is as expected, which is standard normal. We are using a small test set and we cannot expect the the mean and variance to be close to 0 and 1.
