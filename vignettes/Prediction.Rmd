...
We are now ready to create a prediction.
...
```{r eval=TRUE}
preds = (pred > 0.8)
k = tibble('Pred' = preds[,1], 'True'= test_set$fam$pheno_0)
table(k$Pred, k$True)
```
...
## Logistic regression compared to using LT-FH estimates
...
Using the same method as above we can see that the correlation between the genetic liability and PRS found with both logistic regression and linear regression using phenotypes is much lower.


```{r, echo = FALSE}
knitr::include_graphics("prs_true_train_lin01.jpg")
knitr::include_graphics("prs_true_test_lin01.jpg")
```

```{r, echo = FALSE}
knitr::include_graphics("prs_true_train_log.jpg")
knitr::include_graphics("prs_true_test_log.jpg")
```
