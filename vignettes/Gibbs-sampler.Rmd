...
To clarify if a family member is a case we know that the liability of the family member is above a threshold T, given a prevalence K. We the treshold given the standard normal distribution.

$$
T = \Phi^{-1}(1-K)
$$
The liability of person i is given by the conditional distribution $L_p_i \mid L_{p_1}, \cdots, L_{p_{i-1}}, L_{p_{i+1}}, \cdots  L_{p_N} \sim N(\mu_p, \sigma_p)$


To sample from a truncated normal distribution we sample a value from one of the two intervals shown below.
$$
\wedge  \\
\Downarrow
\\
...
$$
(\text{phenotype}_{subject}, \text{phenotype}_{parent1}, \text{phenotype}_{parent2}, \text{phenotype}_{sibling1}, ...,\text{phenotype}_{siblingN})
$$
