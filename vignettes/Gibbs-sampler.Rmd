...
$$
Using this we can sample from a truncated normal distribution. Given the probality of $L_{p_i}$ we write the following intervals.
$$
P(L_{p_i} \leq T \mid L_{p_1}, \ldots, L_{p_{i-1}}, L_{p_{i+1}}, \ldots,  L_{p_N}) = F_{L_{p_i}}(T) \\
\Downarrow
\\
\text{if p is case}: p_{l_p_i} \in [F_{L_{p_i}}(T) , 1 ]\\
\text{else}: p_{l_p_i} \in [0,F_{L_{p_i}}(T)] \\
$$
